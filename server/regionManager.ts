...
export class RegionManager {
  private static regions: ServerRegion[] = [
    {
      id: 'us',
      name: 'US East',
      endpoint: process.env.NODE_ENV === 'production' ? 'wss://us.snakearena.app' : 'ws://localhost:5000',
      playerCount: 0,
    ...
    },
    {
      id: 'eu',
      name: 'Europe',
      endpoint: process.env.NODE_ENV === 'production' ? 'wss://eu.snakearena.app' : 'ws://localhost:5000',
      playerCount: 0,
      maxPlayers: 50 // 10 rooms × 5 players each
    }
  ];
  ...
  public static getCurrentRegion(): 'us' | 'eu' {
    ...
    const forceRegion = process.env.FORCE_REGION as 'us' | 'eu';
    if (forceRegion) {
      return forceRegion;
    }
